...
<div class="row justify-content-between">
  ...
  <div class="col-4"><h2 class="text-center mb-5"><b>Edit Proyect</b> </h2></div>
</div>
@endsection
             ...
             {{-- @method('PUT') --}}   
                <div class="form-group">
                    ...
                    <input
                        ...
                        value="{{ old('project_info') ? old('project_info') : $project->project_info }}"
                        required
                    >
                    @error('project_info')
                ...
                <div class="form-group">
                    ...
                    >
                        ...
                        @foreach ($managers as $manager)
                            <option 
                                value="{{ $manager->id }}" 
                                {{ $project->fk_idproject_manager == $manager->id ? 'selected' : ( old('fk_idproject_manager') == $manager->id ? 'selected' : '') }}
                            >
                                {{$manager->name}}
                            </option>
                        @endforeach
                ...
                <div class="form-group">
                    ...
                    >
                        ...
                        @foreach ($employes as $employe)
                            <option 
                                value="{{ $employe->id }}"
                                {{ $project->fk_idassigned_to == $employe->id ? 'selected' : ( old('fk_idassigned_to') == $employe->id ? 'selected' : '') }}
                            >
                                {{$employe->name}} 
                            </option>
                        @endforeach
                    ...
                    >
                        ...
                        @foreach ($proyectStatuss as $proyectStatus)
                            <option 
                                ...
                                {{ $project->fk_idstatus == $proyectStatus->id ? 'selected' : ( old('fk_idstatus') == $proyectStatus->id ? 'selected' : '') }}
                            >
                ...
                <div class="form-group">
                    <input type="submit" class="btn btn-danger" value="Edit Proyect">
                </div>
            </form>
        </div>
    </div>
